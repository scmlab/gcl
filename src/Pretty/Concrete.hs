...
instance Pretty Stmt where
  ...
  pretty (Assign xs es _) =
    ...
    <+> ":="
    <+> hsep (punctuate comma (map (pretty . depart) es))
  pretty (Assert p _) = lbrace <+> pretty p <+> rbrace
  pretty (LoopInvariant p bnd _) =
        lbrace <+> pretty p
    <+> "bnd:" <+> pretty bnd <+> rbrace
  pretty (Do gdCmds _) = "do"
    <+> align (encloseSep mempty mempty " |" (map pretty gdCmds))
    <+> "od"
  pretty (If gdCmds _) = "if"
    <+> align (encloseSep mempty mempty " |" (map pretty gdCmds))
    <+> "fi"
  pretty (SpecQM _) = "?"
  pretty (Spec _) = "{!  !}"
...
instance Pretty GdCmd where
  pretty (GdCmd guard body _) = pretty guard <+> "->" <+> vsep (map pretty body)
