...
--   fmap f (ExRd m) =
--     ExRd (\s g -> (\(x,s',g') ->
--            (either Left (Right . f) x, s', g')) (m s g))
--
...
--     where bindW _ (Left e,  s', g') = (Left e, s', g')
...
-- rPermute g [x,y] =
--     let (b,g') = random g
--     in (if b then [x,y] else [y,x], g')
-- rPermute g xs = rPermute' g (length xs) xs
--  where rPermute' g _ [] = ([], g)
--        rPermute' g n (x:xs) =
...
-- swapHd :: Int -> a -> [a] -> [a]
-- swapHd 0 x xs = x:xs
-- swapHd i x xs = let (ys,y,zs) = dissect i xs
--                 in y:ys++[x]++zs
--   where dissect _ [] = error "cannot happen"
--         dissect 1 (y:ys) = ([],y,ys)
--         dissect n (y:ys) = let (ws,w,zs) = dissect (n-1) ys
--                            in (y:ws,w,zs)
