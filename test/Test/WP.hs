...
tests :: TestTree
tests = testGroup "Weakest Precondition" [simple, assertions, if']
...
simple = testGroup
  ...
  , testCase "spec" $ do
    ...
    actual @?= Right
      ( []
      , [Specification 0 (Assertion false NoLoc) (Assertion false NoLoc) NoLoc]
      )
  ]
...
run text =
  REPL.runREPLM
    $   (\((_, y), z) -> (map toNoLoc y, map toNoLoc z))
    <$> (   REPL.scan "<test>" (text)
