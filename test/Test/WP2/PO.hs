...
statements = testGroup
    ...
    $   run
            ...
            \skip         \n\
            \{ 0 = 0 }    \n"
    @== poList [PO 0 (assertion true) (assertion (0 === 0)) (AtSkip NoLoc)]
    , testCase "abort"
    $   run
            "{ True }     \n\
            \abort        \n\
            \{ 0 = 0 }    \n"
    @== poList [PO 0 (assertion true) (Constant false) (AtAbort NoLoc)]
    , testCase "assignment"
    $   run
            "{ True }     \n\
            \x := 1       \n\
...
assertions = testGroup
    ...
    $   run
            ...
            \{ True }     \n"
    @== poList
            [ PO 0 (assertion true)  (assertion false) (AtAssertion NoLoc)
            , PO 1 (assertion false) (assertion true)  (AtAssertion NoLoc)
            ]
...
if' = testGroup
    ...
    $   run
            ...
            \{ 0 = 3 }\n"
    @== poList
            [ PO 0 (assertion (0 === 0)) (guardIf (0 === 1)) (AtIf NoLoc)
            , PO 1
                 (Conjunct [assertion (0 === 0), guardIf (0 === 1)])
                 (guardIf (0 === 2))
                 (AtIf NoLoc)
...
loop = testGroup
    ...
    $   run
          ...
          \od                     \n\
          \{ 0 = 0 }              \n"
    @== let inv = loopInvariant (0 === 1) "A"
        in  poList
                [ PO 0
                     (Conjunct [inv, Negate (guardLoop (0 === 2))])
                     (assertion (0 === 0))
                     (AtLoop NoLoc)
                , PO 1 (Conjunct [inv, guardLoop (0 === 2)]) inv (AtSkip NoLoc)
                , PO 3
    ...
    @== let inv = loopInvariant (0 === 1) "A"
        in
            poList
                ...
                , PO
                    4
                    (Conjunct [inv, guardLoop (0 === 2), guardLoop (0 === 3)])
