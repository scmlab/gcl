...
expression :: TestTree
expression = testGroup "Expressions" $ map
    ...
    , RightCase "numeric 2" "A + X * Y" $ bin
        ...
        (1 <-> 9)
    , RightCase "numeric 3" "(A + X) * Y % 2" $ bin
        Mul
        (at 9)
...
type' = testGroup "Types" $ map
    ...
    , RightCase "function types 2" "(Char -> Int) -> Int" $ TFunc
