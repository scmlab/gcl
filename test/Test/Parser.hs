...
expression =
  testGroup
    ...
    [ testCase "literal (numbers)" $ run "1",
      ...
      testCase "quant 6 (sum)" $ run "x = <| Σ i : 0 < i < n : i |>",
      testCase "quant 7 (forall)" $ run "x = <| ∀ i : 0 < i < n : i > 0 |>",
      testCase "quant 8 (exists)" $ run "x = <| ∃ i : 0 < i < n : i > 0 |>",
      testCase "quant 9 (hash)" $ run "x = <| # i : 0 < i < n : F i > 0 |>",
